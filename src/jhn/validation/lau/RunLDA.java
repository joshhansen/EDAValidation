...
public class RunLDA {
	...
	public static void generate(String datasetName, int[] topicCounts, int numRuns) throws IOException, InterruptedException {
		...
		for(int numTopics : topicCounts) {
			for(int run = 0; run < numRuns; run++) {
				...
				cmd.append(jhn.validation.Paths.lauKeysFilename(datasetName, numTopics, run));
				...
				cmd.append(jhn.validation.Paths.lauDocTopicsFilename(datasetName, numTopics, run));
	...
	private static void ensureDirs(String dataset) {
		new File(jhn.validation.Paths.lauKeysDir(dataset)).mkdirs();
		new File(jhn.validation.Paths.lauStateDir(dataset)).mkdirs();
		new File(jhn.validation.Paths.lauDocTopicsDir(dataset)).mkdirs();
	}
	
	public static void main(String[] args) throws IOException, InterruptedException {
//		String datasetName = "reuters21578_noblah";
//		String datasetName = "state_of_the_union";
		String datasetName = "sotu_chunks_subset1";
