...
public class RunLau implements AutoCloseable {
	...
	public void init() throws Exception {
		...
		Log log = new Log(System.out, Paths.lauLogFilename(dataset));
...
//		OrderedTitleSearcher ts3 = new GoogleTitleSearcher(conf.getInt(Options.TITLE_SEARCHER_TOP_N));
	...
	private void runLau(int numTopics, int run) throws Exception {
		String keysFilename = Paths.lauKeysFilename(dataset, numTopics, run);
		String topicLabelsFilename = Paths.lauTopicLabelsFilename(dataset, numTopics, run);
...
//		final String datasetName = "sotu_chunks_subset1";
